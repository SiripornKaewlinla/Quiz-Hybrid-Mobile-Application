...
export default function LoginScreen({ navigation }: any) {
  ...
  const [email, setEmail] = useState("siriporn.ka@kkumail.com"); // default
  const [password, setPassword] = useState("Password123");         // default
  const [showPassword, setShowPassword] = useState(false);

  const handleLogin = async () => {
    try {
      await login(email, password);
      navigation.replace("Home"); // เปลี่ยนหน้า Home
    } catch {
  ...
  return (
    <View style={[styles.container, { paddingHorizontal: width * 0.08 }]}>
      ...
      <Text style={styles.note}>
        ตัวอย่างบัญชี: siriporn.ka@kkumail.com / Password123
      </Text>
    </View>
  );
}
